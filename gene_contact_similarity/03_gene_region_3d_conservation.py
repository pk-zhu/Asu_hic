...
输出 -> results/gene_3d/{matrix_res}/
  gene_3d_conservation_Asu_{Ath,Aar}.tsv   per-gene
  gene_3d_summary.tsv                       sT vs sA 汇总
"""
import os
import sys
import numpy as np
...
OUTPUT_BASE = os.path.join(BASE_DIR, "04.cds_synteny_3d_conservation/results/gene_3d")

# 支持的配置: (matrix_resolution, orth_bins_resolution)
MATRIX_CONFIGS = [
    (2000, 5000),   # 2k matrix, 5k orth_bins for mapping
...
def run_matrix_resolution(matrix_res, orth_res):
    ...
    comparisons = [
        ("Asu_Ath", f"Asu_Ath_orth_bins_{orth_res}.tsv", "Ath", ST_TO_ATH),
        ("Asu_Aar", f"Asu_Aar_orth_bins_{orth_res}.tsv", "Aar", SA_TO_AAR),
    ]
    ...
    for comp, fname, par_sp, cmap_def in comparisons:
        ...
        asu_clr = cooler.Cooler(f"{SINGLE_DIR}/Asu/Matrix/Final/Asu.mcool::resolutions/{matrix_res}")
        ...
        sub_label = "sT" if comp == "Asu_Ath" else "sA"
    ...
    import csv
    sout = os.path.join(output_dir, "gene_3d_summary.tsv")
